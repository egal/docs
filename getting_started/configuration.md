...
В разделе описана конфигурация сервисов и основных
контейнеров с описанием переменных окружения.
...
Конфигурация сервиса задается через переменные окружения. Они расположены:
* в файле .env в корне сервиса
* в файле docker-compose.yml для конкретного сервиса.

Пример конфигурирования сервиса через
...
В данном случае все происходит также как в типичном проекте на lumen или laravel. В разделе `environment` находятся переменные окружения для контейнера сервиса. С их помощью настраивается приложение в контейнере. Подробнее здесь
...
позволяют настроить контейнер сервиса.
[Docker Compose](https://docs.docker.com/compose/)

> Конструкция вида `${DB_DATABASE:-5432}` означает,
> что подставляется переменная окружения DB_DATABASE, либо значение по
> умолчанию `5432`.

> Если значение переменной (в т.ч. по умолчанию)
> не определено, используется пустое значение.
...
| WAIT_HOSTS                  | Перечень хостов, перечисленных через запятую, который должен ждать текущий контейнер для загрузки. | Хост вида: project-rabbitmq:5672             |
...
| RABBITMQ_PORT               | Содержит порт, где работает RabbitMQ.                                                            | Целое неотрицательное число                 |
...
| DB_PORT                     | Порт подключения к БД.                                                                                 | Целое неотрицательное число                 |
...
Конфигурация проекта - совокупность конфигураций контейнеров.
Содержит конфигурации основных
сервисов и сервисов, обеспечивающих их работу. К последним относятся:
- rabbitmq
- database
...
Часть переменных выносится в .env файл в корне
проекта. Он используется приложением docker-compose, а переменные
подставляются в файле docker-compose.yml при build и запуске
контейнеров.

| Название переменной   | Описание                                                                   | Возможные значения           |
|:----------------------|:---------------------------------------------------------------------------|:-----------------------------|
| PROJECT_NAME          | Название проекта. Подставляется  <!-- Подставляется что? -->                                          | Строка                       |
| RABBITMQ_PORT         | Пароль для подключения к RabbitMQ.                                         | Целое неотрицательное число |
| RABBITMQ_MANAGER_PORT | Активируемые плагины RabbitMQ. Перечисляются через запятую. | Целое не отрицательное число |
| DATABASE_PORT         | Список баз данных для других сервисов в проекте.         | Целое не отрицательное число |
