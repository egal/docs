...
Для использования экшена отдельно от модели инициализируется новый экземпляр класса с указанием параметров.
...
В переменных *orders* и *filter* хранятся параметры передаваемые с запросом. Вид параметров всегда должен быть таким как указано.
...
Когда параметры определены, вызывается метод **socketConnect()** для соединения с сервером через сокеты или **axiosConnect()** для соединения через *http* запрос.

Запрос определенных полей возможен только с использованием модели.

**getItem:**
запрос getItem похож на getItems, но дополнительно к параметрам указывается id сущности с помощью метода setId()
...
Использование экшенов, вызываемых через модель (передача параметров отличается от отдельного использования экшенов):
...
в переменных *orders* и *filter* хранятся параметры передаваемые с запросом. Вид параметров должен быть таким, как указано.
...
запрос getItem похож на getItems, но дополнительно к параметрам указывается id перед типом соединения.
 ```
messageTest.actionParameters.setId(12)
```
