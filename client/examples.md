...
В переменных *orders* и *filter* хранятся параметры, которые передаются вместе с запросом.
Вид этих параметров фиксированный и должен быть таким, как указано.

Массив *orders* хранит массивы с указанием названия сортируемого поля и порядком сортировки
...
Когда параметры определены, вызывается метод **socketConnect()** для соединения с сервером через
сокеты или **axiosConnect()** для соединения посредством *http* запроса.
...
Далее указываем URL для связи с сервером с помощью метода модели **setBaseUrl**.
Так как в зависимости от типа связи (сокеты или _axios_), URL будет отличаться, также указываем тип связи:
```
/**
...
В переменных *orders* и *filter* хранятся параметры, которые необходимо передать вместе с запросом.
Вид этих параметров фиксированный и должен быть таким, как указано.
