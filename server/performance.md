...
* web-service: 
  ...
  * Количество воркеров сервиса: 8
  * image: egalbox/web-service:v2.1.0
* first-service:
  * Количество сервисов: 1
  * Количество воркеров сервиса: 1
...
#### Docker services scaling

В данном разделе тестирование нагрузки проводилось на самых простых примерах и не требовало большего числа
инстансев сервисов. Однако, если в процессе разработки вы столкнетесь с ситуацией, когда сервис не будет успевать обрабатывать
требуемое количество входящих запросов, стоит обратить внимание на возможность увеличения числа инстансов сервиса.
Правильная настройка количества инстансов всех частей приложения может в разы увеличить пропускную способность и общую производительность приложения.
...
Важно учитывать, что каждый поднятый инстанс сервиса запускает одного воркера. При включении автоконфигурирования количество воркеров веб-сервиса 
зависит от CPU, см. [описание веб-сервиса](/server/services/web_service.md#Образ-egalbox/web-service).
