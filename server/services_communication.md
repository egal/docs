...
***
Комментарии:
- В классе '\Egal\Core\Communication\Response' отсутствует метод 'hasError' (строки 65, 89);
- Свойство 'isConnectionOpened' (строка 82) - приватное, нельзя к нему обратиться из дочернего класса;
- Не ясно, что имеется в виду под 'Отключена ли авторизация' (строка 20);
- Тут, возможно, придираюсь, но википедия говорит "тег" (строка 26) :))
- Не ясно, зачем использовать методы 'openConnection' и 'closeConnection', т.к. запросы можно отправить не используя их.
